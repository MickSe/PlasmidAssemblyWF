...
nextflow run $wf \
        ...
        --db_directory /path/to/database/built/during/installation \
        --threads 4
...
WARNING: If a ref_seq_sheet is given but no corresponding sample ID is found, report will not be generated.
	This is an edge case that should be solved in the future to allow report output even 

All output files are generated in the directory where the command was executed. Report, annotation in gbk, feature table in csv and assembly in fasta are separated in a different subdirectory for each barcode, while all other files (bed, vcf, json) are generated in a subdirectory called "other".
...
WARNING: if the content of --ref_seq_sheet does not correspond to the expected sample names (barcodes, or sample IDs if --sample_sheet was used), we fall in an edge case where the worflow does not fail but ignores some processes like report generation. This is to be corrected in the future (bug probably due to the content of the variant calling output channel, that remains empty even though the process is being called).
...
Tools used for variant calling (minimap2, samtools, sniffles) were added in the docker image originally used by the workflow, and the new image (wf-clone-validation-with-sniffles) has to be created from the container in /EPI2ME_pipeline/docker_image/wf-clone-validation-with-sniffles.tar
